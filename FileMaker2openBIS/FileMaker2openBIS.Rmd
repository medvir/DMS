...
FileMakerSamples = read_csv(FileMakerSamplesFile, col_names = FALSE) %>%
        ...
        ungroup %>%
        select(identifier, parents, experiment, default_space, SAMPLE_ID, SAMPLE_NAME, SAMPLE_PLATE, SAMPLE_WELL, INDEX_1, INDEX_2, I7_INDEX_ID, I5_INDEX_ID, DESCRIPTION)

FileMakerSamples[is.na(FileMakerSamples)] = ""

FileMakerSamples
```
### Write MISEQ_SAMPLES upload file
```{r, echo=FALSE, message=FALSE}
FileMakerSamples %>%   
...
## RESISTANCE_TEST samples for each MISEQ_SAMPLE in Project RESISTANCE
```{r, echo=FALSE, message=FALSE}
FileMakerResistanceTests = FileMakerSamples %>%
        filter(experiment == "/IMV/RESISTANCE/MISEQ_SAMPLES") %>%
        mutate(experiment = "/IMV/RESISTANCE/RESISTANCE_TESTS") %>%
        mutate(parents = identifier) %>%
        mutate(identifier = paste0(identifier, "_RESISTANCE")) %>%
        mutate(SAMPLE_NAME = sub("_L001_R1_001.fastq", "", SAMPLE_NAME )) %>%
        select(identifier, parents, experiment, default_space, SAMPLE_NAME) %>%
        mutate(VIRUS = "", TARGET_REGION = "", GENOTYPE = "", VIRAL_LOAD = "")

FileMakerResistanceTests
```

### Write RESISTANCE_TEST upload file
```{r, echo=FALSE, message=FALSE}
FileMakerResistanceTests %>%   
        #filter(substr(parents, 1, 2) == "17") %>% ### filter for year
        write_tsv(., paste0(path, "SAMPLE_RESISTANCE_TEST.tsv"))
```
